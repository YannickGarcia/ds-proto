...
export interface BarChartProps {
  ...
  stacked?: boolean;
  /**
   * Pins the top of the value domain instead of deriving it from the data.
   * Use for 100%-stacked charts, which should fill the plot exactly.
   */
  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      resolveDomain: (dataKeys) => {
        let max = 0;
        for (const d of data) {
          // Stacked series share one axis, so the domain has to cover the
          // stack total — sizing it to the tallest single series lets tall
          // stacks paint outside the plot.
          if (stacked) {
            let sum = 0;
        ...
        if (valueDomainMax !== undefined) {
          return [0, valueDomainMax];
        }
        return [0, (max || 100) * 1.1];
      },
      nice: valueDomainMax === undefined,
    });
  }, [data, innerHeight, isHorizontal, lines, stacked, valueDomainMax, valueScale]);
